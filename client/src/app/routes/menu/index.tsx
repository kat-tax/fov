      ...
      <FocusContext.Provider value={focusKey}>
        <ScrollView ref={ref} contentContainerStyle={{flexGrow: 1}}>
          <View style={styles.root}>
            ...
            {__DEV__ &&
              <MenuGroup label={t`Dev Mode`} closed actions={[
                {
                  id: 'toggle-dev-mode',
                  icon: showScanner ? 'ph:scan-fill' : 'ph:scan',
                  label: t`Toggle Dev Mode`,
                  visible: showScanner,
                  onPress: () => {
                    const toggle = !showScanner;
                    scan({showToolbar: toggle});
                    toast({title: `React scan ${toggle ? 'enabled' : 'disabled'}`, preset: 'done'});
                    setShowScanner(toggle);
                  },
                },
              ]}>
                <MenuItem
